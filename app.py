...
class Manga:
    def __init__(self, slug: str, meta: dict, dirpath: Path, chapters):
        ...
        # Reader mode: manga (default) vs manhwa/webtoon
        mode = str(meta.get("reader_mode", meta.get("format", "manga"))).lower()
        self.reader_mode = mode
        self.is_manhwa = (
            mode in ("manhwa", "webtoon", "vertical")
            or any(str(t).lower() == "manhwa" for t in self.tags)
        )
...
def robots_txt():
    txt = "\n".join([
        ...
        "Allow: /",
        "",
        f"Sitemap: https://kumoscans.com/sitemap.xml",
    ])
